$(function(){
	$(document).on('click', '.addTweetBtn', function(){
		...
		$.post('http://localhost/WebDeveloper/Twitter-Clone/core/ajax/tweetForm.php', function(data){
			$('.popupTweet').html(data);
			$('.closedTweetPopup').click(function(){
				$('.popup-tweet-wrap').hide();
				$('.status').removeClass().addClass('status');
				$('.hash-box').removeClass().addClass('hash-box');
				$('#count').attr('id', 'count');
			});
	...
	$(document).on('submit', '#popupForm', function(e){
		...
		$.ajax({
			url: "http://localhost/WebDeveloper/Twitter-Clone/core/ajax/addTweet.php",
			type: "POST",
			data: formData,
